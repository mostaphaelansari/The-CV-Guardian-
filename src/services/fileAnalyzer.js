...
class FileAnalyzer {
    ...
    async analyze(fileBuffer, fileName, mimeType) {
        ...
        if (hasCritical) {
            report.score = Math.max(report.score, 90); // Critical findings must have high score
            report.riskLevel = 'critical';
        } else if (hasHigh) {
            report.score = Math.max(report.score, 75); // High severity findings must have high score
